package com.example.feature_auth.screen.registration
...
fun RegistrationScreen(
    ...
    val intentChannel = remember { Channel<RegistrationContract.ViewIntent>(Channel.UNLIMITED) }
    LaunchedEffect(Unit) {
    ...
    LaunchedEffect(key1 = viewEvent) {
        when (val event = viewEvent) {
            is RegistrationContract.SingleEvent.NavigateToSignIn -> {
                Timber.tag("navigate").d("NavigateToSignInScreen")
                navigator.navigate(
                    route = FeatureAuthRoute.EnterEmail.INSTANCE.routeName,
                    navOptions = navOptions {
                        popUpTo(FeatureAuthRoute.Registration.INSTANCE.routeName) {
                            inclusive = true  // Удалить экран регистрации из стека
                        }
                    }
                )
            }

            is RegistrationContract.SingleEvent.ShowError -> {
    ...
    Scaffold { paddings ->
        ConstraintLayout(
            ...
            InputContainer(
                ...
                viewState = viewState,
                onEmailEntered = {
                    intentChannel.trySend(RegistrationContract.ViewIntent.OnEmailChanged(it))
                },
                onActionClicked = {
                    intentChannel.trySend(RegistrationContract.ViewIntent.Registration)
                    Timber.tag("login___________").d("trySend")
                }
            )
        }
        ShowRegistrationError(isVisible = viewState.isEmailNotCorrect)
    }
...
private fun InputContainer(
    ...
    viewState: RegistrationContract.ViewState,
    ...
    Column(
        ...
        Text(
            text = stringResource(com.example.locale.R.string.auth_registration),
...
@Composable
private fun ShowRegistrationError(isVisible: Boolean) {
    if (isVisible) {
        registrationErrorToast(message = stringResource(com.example.locale.R.string.auth_email_registration_error))
    }
}

@Composable
fun registrationErrorToast(message: String) {
